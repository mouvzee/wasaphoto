...
func (db *appdbimpl) GetUserProfile(profileUserID int, userID int) (Profile, error) {
	...
	err = db.c.QueryRow(query_GETCOUNTFOLLOWINGS, profileUserID).Scan(&profile.Following)
	if err != nil {
		return profile, err
	}

	err = db.c.QueryRow(query_GETCOUNTFOLLOWERS, profileUserID).Scan(&profile.Follower)
	if err != nil {
		return profile, err
	}
	...
	err = db.c.QueryRow(query_GETCOUNTPOSTS, profileUserID).Scan(&profile.PostsCount)
	if err != nil {
		return profile, err
	}
